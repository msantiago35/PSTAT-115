---
title: "Homework 5 - PSTAT 115"
author: "Marissa Santiago"
...
logistic_fit <- sampling(log_mod,data=list(N=n,x=x,y=y),refresh=0)
...
samples <- samples # mh_logistic(c(0, 0), 1000, 10000)
...
```
The standard deviation of the 'true skill' is smaller. This makes sense because there is less variation of the true skill given a longer period of time, rather than just a month. The empirical data's small sample sizes introduce much more sampling variability.
...
```
The complete pooling gives a lower error therefore being the better estimate.
...
As $\tau \to \infty$ the posterior mean estimate will approach no pooling. The true skill for the baseball players would be very large meaning everyone would have differing skill levels.
As $\tau \to 0$ the posterior mean estimate will approach complete pooling. This would be approaching the assumption that each player has the same skill level.
...
results <- rstan::sampling(baseball_stan_model,
...
pm <- inv_logit(theta_samples) %>% colMeans()
...
```
RMSE of partial pooling has the lowest error and is lower than both RMSE for complete pooling and no pooling.

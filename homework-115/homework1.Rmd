...
date: "__Due on Sunday, January 17, at 11:59 pm__"
output: pdf_document
urlcolor: blue
---
...
$$\mathcal{L} ( \lambda ) =  \frac{e^{-\lambda n} {\lambda  ^{ \sum\nolimits_{i=1}^{n} y_i}}}{ \prod_{i=1}^{n} y_i!}$$


$$\mathcal{L} ( \lambda ) = e^{-\lambda n} {\lambda  ^{ \sum\nolimits_{i=1}^{n} y_i}} $$
...
ggplot(text_tb,aes(x=chapter, y= dark_counts)) + geom_bar(stat="identity")
```
...
x <- dark_counts
L <- function(lambda){
  sum(x)*log(lambda)-37*lambda
}

lambda_mle = mean(x)
lambda_mle

plot <- ggplot(data = data.frame(lambda = 0), mapping = aes(x=lambda))+stat_function(fun = L)+xlim(0,40)+scale_y_continuous(name = "Likelihood")
plot+geom_vline(xintercept = lambda_mle, color = 'red')
```
...
Our interpretation of $\lambda$ in this model is the expectation of Yi which is the  average number of times the word *dark* is used every 1000 words. With vi being the total number of words in each chapter, we can interpret vi/1000 as the frequency of the word *dark* per 1000 words.
...
Likelihood:
$$\mathcal L\left(\lambda * \frac{\nu_i}{1000}; y_1,...,y_n\right) = \displaystyle\prod_{i=1}^n exp\left(-\lambda*\frac{\nu_i}{1000}\right)\displaystyle\frac{1}{y_i!}\lambda^{y_i}$$

$$\mathcal = exp\left(-\frac{n\lambda}{1000}\sum_{i=1}^n \nu_i\right)\displaystyle\frac{\lambda^{\sum_{i=1}^ny_i}}{y_1!y_2!...y_n!}$$

Log-Likelihood:
$$\mathcal \ell\left(\lambda* \frac{\nu_i}{1000}; y_1,...,y_n\right) = \displaystyle\frac{-n\lambda}{1000}\sum_{i=1}^n \nu_i + log(\lambda)\displaystyle\sum_{i=1}^n y_i - \displaystyle\sum_{i=1}^n log(y_i!) \propto -n\lambda\sum_{i=1}^n \nu_i + log(\lambda)\displaystyle\sum_{i=1}^n y_i$$

Derivative with respect to Lambda:
$$\mathcal\displaystyle\frac{d\ell}{d\lambda} = \left(-n\lambda\sum_{i=1}^n \nu_i + log(\lambda)\displaystyle\sum_{i=1}^n y_i\right)' = -n\sum_{i=1}^n \nu_i + \frac{1}{\lambda}\sum_{i=1}^n y_i$$

Set Derivative to 0:
$$\mathcal-n\displaystyle\sum_{i=1}^n \nu_i + \frac{1}{\lambda}\sum_{i=1}^n y_i = 0$$

$\frac{1}{\lambda}\sum_{i=1}^n y_i = n\sum_{i=1}^n \nu_i$

$\lambda = \frac{\sum_{i=1}^n y_i}{n\sum_{i=1}^n \nu_i}$
...
```
This means that the mle is the estimate for how many times the word *dark* would appear per 1000 words.
...
vi <- chapter_lengths
L <- function(lambda){
  lambda* -sum((vi)/1000)+sum(x)*log(lambda*prod((vi)/1000))
}
plot2 <- ggplot(data=data.frame(lambda=lambda_mle),mapping = aes(x=lambda))+stat_function(fun=L)+xlim(0,5)+scale_y_continuous(name = 'Likelihood')
plot2+geom_vline(xintercept = lambda_mle,color='red')
```
...
It may be unreasonable to assume the rate of _dark_ usage is the same in all chapters because not all chapters contain the same story elements tht make up the overall plot. In one chapter the setting may be during the nighttime which would cause 'dark' to be used more frequently than if a chapter was set during the daytime.
...
```
Yes there are outliers that don't seem to be reflected in our simulated histogram. In the observed data, there seems to be a more spread out histogram of the dark counts, with there being an outlier near 25 which it is not seen in our simulated histogram.
...
# Maximum likelihood estimate
lambda_hats <- x/(chapter_lengths/1000)

darkest_chapter <- -max(lambda_hats)
darkest_chapter

# Make a bar plot of the MLEs, lambda_hats
barplot(lambda_hats,names.arg = c(1:length(lambda_hats)),main = 'Rate of dark usage (per 1000 words) per chapter', xlab = 'Chapter',ylab = 'Rate of dark usage per 1000 words')
```
...
$y_i$ is the number of times *dark* appears in the ith chapter.

$\lambda_i$ is the rate that *dark* is used in the ith chapter.

$\lambda_i$ has a Gamma distribution with known parameters $\alpha$ and $\beta.$

$Y_i$ has a Poisson distribution with parameter $\lambda_i$ which has a Gamma distribution as stated above.

We would first have to simulate values for $\lambda_i$ in order to generate $Y_i$.
...
for(i in 1:1000){
  lambda_i <- rgamma(1000,10,1)
  Y_i <- rpois(1000,lambda_i)
}
counts <- Y_i
print(mean(counts))
print(var(counts))
...
```
The variance is greater than the mean, so it is overdispsersed. Since $\lambda$ is a random variable in this case, the variance will be greater. 
...
Known, Var $= 0$:  $\alpha$ , $\beta$ , $y_1, ..., y_n$
